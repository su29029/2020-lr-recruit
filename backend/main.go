...
func loginHandler(c *gin.Context) {
	...
	if c.Request.URL.Path == "/api/admin/login" {
		if way.Md5(user.UserID) == "a0e6ff00131e8f13fe9d4e65044a8b1a" && user.Passwd == "29d1f33f372bcdc9ed5c0a29554d975f" {
			way.SetSession(c, "user", user.UserID, 3600*24*7)
...
func guestGetProfileSolvesHandler(c *gin.Context) {
	...
	var rows *sql.Rows
	var err error
	var field []string = []string{"web", "UI", "机器学习", "安卓", "网络安全", "基础"}
	//获取登录用户
	userID = c.Param("userID")
	...
	for _, f := range file {
		if !f.IsDir() {
			...
			if id == problemID { // 找到了需要下载的文件
				...
				flag--
				c.Writer.Header().Add("filename", fileName)
...
func adminUpdateUserScoreHandler(c *gin.Context) {
	...
	sqlStr = "select round(sum(solved_problem_score * magnification)) as 'androidScore' from problem_solved_table where solved_problem_field = 3 and userID = ? group by userID"
	row = utils.Db.QueryRow(sqlStr, userSolved.UserID)
	row.Scan(&user.AndroidScore)
...
func adminNewProblemHandler(c *gin.Context) {
	...
	sqlStr := "insert into problem_table (problem_title, problem_content, problem_score, current_score, problem_category, problem_solved, flag, is_show, first_blood) values ('', '', 0, 0, ?, 0, '', 0, '')"
...
func adminGetAllUsersHandler(c *gin.Context) {
	...
	sqlStr = "select userID, round(sum(solved_problem_score * magnification)) as 'androidScore' from problem_solved_table where solved_problem_field = 3 group by userID order by userID"
	...
	if err == nil {
		...
		for j := 0; j < len(users); j++ {
			for m := 0; m < i; m++ {
				if users[j].UserID == tmpUser[m] {
					users[j].AndroidScore = tmpScore[m]
					break
				}
			}
		}
...
func guestUploadHandler(c *gin.Context) {
	...
	newDst := path.Join("/app/upload", userID, "new_"+strconv.Itoa(problemID)+"_"+userID+"_"+problemCategory+"_"+strings.Replace(problemTitle, " ", "_", -1)+suffix)
	oldDst := path.Join("/app/upload", userID, strconv.Itoa(problemID)+"_"+userID+"_"+problemCategory+"_"+strings.Replace(problemTitle, " ", "_", -1)+suffix)
	if res, _ := way.IsFileExist(oldDst); res { // 如果老文件存在，则删除它
		os.Remove(oldDst)
	}
	err = c.SaveUploadedFile(file, newDst)
	if err != nil {
		fmt.Println(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"msg": "error",
