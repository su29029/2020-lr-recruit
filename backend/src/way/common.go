...
func CheckInformation(information interface{}) bool {
	...
	case model.Profile:
		...
		if len(profile.UserID) < 3 || len(profile.UserID) > 20 {
...
func checkStudentID(studentID string) bool {
	re := regexp.MustCompile(`^((2020\d{9})|(2019\d{9})|(2018\d{9}))$`)
	result := re.MatchString(studentID)
	return result
}

func checkPhoneNumber(tel string) bool {
	re := regexp.MustCompile(`^((0\d{2,3}-\d{7,8})|(1[358479]\d{9}))$`)
...
func GuestGetProblemsByField(FieldID int, userID string) ([]model.Problems, error) {
	...
	var rows *sql.Rows
	var err error
	sqlStr = "select problem_id, problem_title, problem_score, current_score, problem_category, problem_solved from problem_table where problem_category = ? and is_show = 1"
...
func AdminGetProblemsByField(field int) ([]model.Problems, error) {
	var problems []model.Problems
	var problem model.Problems
	sqlStr := "select problem_id, problem_title, problem_content, problem_score, current_score, problem_solved, flag, is_show from problem_table where problem_category = ?"
	rows, err := utils.Db.Query(sqlStr, field)
...
func SplitUploadFileNameToUploadType(uploadFileName string) model.Upload {
	...
	split := strings.Split(uploadFileName, "_")
	upload.ProblemID, _ = strconv.Atoi(split[0])
	upload.UploadUser = split[1]
